// server.js — Optimized scanner with caching & efficient progress tracking
...
// --- CONFIG ---
const SCAN_DELAY = parseInt(process.env.SCAN_DELAY || "500", 10); // ms delay per map
...
async function getRows(sql, params = []) { const res = await pool.query(sql, params); return res.rows; }
async function getRow(sql, params = []) { const res = await pool.query(sql, params); return res.rows[0]; }
...
// --- Get all beatmaps (cached) ---
async function getAllBeatmaps(forceRefresh = false) {
  if (cachedBeatmaps.length > 0 && !forceRefresh) return cachedBeatmaps;
  let allBeatmaps = [];
  let page = 1;
  while (true) {
    try {
  ...
  cachedBeatmaps = allBeatmaps;
  log(`📊 Total beatmaps fetched: ${cachedBeatmaps.length}`);
...
async function updateLeaderboards() {
  ...
  const beatmaps = await getAllBeatmaps(true); // refresh cache
  scanProgress.total = beatmaps.length;
  ...
  if (priorityBeatmaps.length > 0) {
    ...
    for (const bm of priorityBeatmaps) {
      scanProgress.currentBeatmap = bm.beatmap_title;
  ...
  if (lastId) {
    ...
    else { await saveProgress("last_beatmap_id", null); startIndex = 0; }
  }

  const mapsToScan = beatmaps.slice(startIndex);
  for (let i = 0; i < mapsToScan.length; i++) {
    const bm = mapsToScan[i];
    scanProgress.currentBeatmap = bm.title;
    scanProgress.processed = startIndex + i + 1;
    scanProgress.percentage = ((scanProgress.processed / scanProgress.total) * 100).toFixed(2);
    await limiter.schedule(() => fetchLeaderboard(bm.id, bm.title));
    await saveProgress("last_beatmap_id", bm.id);
    await sleep(SCAN_DELAY);
  }
  log("✅ Finished scan cycle");
}
...
// New efficient progress endpoint
